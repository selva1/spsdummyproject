...
if($search_val!=""){
 $wherecaseselect = " and fpc.doctor_id='".$search_val."' "; 
	...
	if(isset($_POST['allBandVals']) && $_POST['allBandVals']!=""){
		...
		$allBandValsWhere ="and p.cat_id IN ($allBandVals)";
    ...
    if(isset($brand_id) && $brand_id!=""){
    	if($seo1!=""  && $seo2!=""){
		 $cattovalueWhere =" and p.Brand IN($brand_id) and p.cat_id IN($cat_id)";
		}else{
		 $cattovalueWhere =" and p.Brand IN($brand_id)";
		}
	...
	where p.ProductID!=''  ".$cattovalueWhere." ".$allcolorValsWhere."  ".$allBandValsWhere."  ".$priceWhere." AND `product_status`=0  order by p.ProductID ASC";
...
while($result = mysqli_fetch_array($result_final)){
		...
		<li>
			...
			<img src="<?php echo SITE_URL;?>assets/img/No-Image.jpg"  width="250" height="250" alt="No-Image">
			<?php } else { ?>
