...
$userData = $_REQUEST['userCreationRequest'];

$google_email = $userData['email'];
$google_name = $userData['name'];
$google_fname = $userData['firstName'];
$google_lname = $userData['lastName'];
$google_login_type = $userData['loginType'];
$currentDate = date("Y-m-d H:s");

$user_data=[];
$query="select id,email,user_name from hm_users where email='$google_email' AND login_type='$google_login_type'";
...
if (empty($user_data)) {
    $stmtQaf = "INSERT INTO hm_users (`user_name`, `email`,accepted_terms,login_type,created_on) 
                VALUES 
                ('$google_name','$google_email','0','$google_login_type','$currentDate')";
   ...
   if ($results) {
       ...
       $_SESSION['resultEmail'] 	= $google_email;
       ...
       $headers1 = "From: fdfsd Newsletter <$from>\n";
