...
if($search_val!=""){
 $wherecaseselect = " and fpc.doctor_id='".$search_val."' "; 
	...
	$query = "SELECT `id`, `color`, `color_status` FROM `tbl_colors` where id!='' order by id ASC";
...
while($result = mysqli_fetch_array($result_final)){
        ...
        <tr>
          <td><?php echo $i; ?></td>
          <td><?php echo htmlentities($color); ?></td>
          <td><?php 
          if($color_status=="0"){
		  	echo "Active"; 
		  }else{
          ...
          ?></td>
          <td> <a href="add_color.php?editId=<?php echo $id; ?>">Edit</a> | <a href="javascript:void(0);" onclick="deleteColor(<?php echo $id; ?>);">Delete</a></td>
      </tr>
