function load(page,serch_box_val,str_check,let_search,type_vals,ids_indst){
	 ...
	 $("#loader").fadeIn('slow');
		$.ajax({
			url:'get_all_static_page_list.php?action=ajax&page='+page+'&search_val='+serch_box_val+'&str_check_box='+str_check+'&searchd='+let_search+'&type_val='+type_val+'&ids_indst='+indst_val,
...
function deletePage(Ids){
	var site_url              = jQuery("meta[name='siteurl']").attr("content");
		$.post("ajax_all_add_edits.php" , {command:'deleteStaticPage',Ids:Ids} , function(data,status)	{
