function load(page,serch_box_val,str_check,let_search,type_vals,ids_indst){
		if(typeof(type_vals)=="undefined"){
			var type_val ="";
		} else {
			var type_val =type_vals;
		}
		if(typeof(ids_indst)=="undefined"){
			var indst_val ="";
		} else {
			var indst_val =ids_indst;
		}
	 $("#loader").fadeIn('slow');
		$.ajax({
			url:'get_all_categoryies_list.php?action=ajax&page='+page+'&search_val='+serch_box_val+'&str_check_box='+str_check+'&searchd='+let_search+'&type_val='+type_val+'&ids_indst='+indst_val,
			success:function(data){
				$("#outer_div").html(data).fadeIn('slow');
				$("#loader").fadeOut('slow');
			}
		})
	}
	
function fefine_search(){
var refinesearch = document.getElementById("refinesearch").value;
	$("#loader").fadeIn('slow');
		$.ajax({
			url:'get_all_categoryies_list.php?action=ajax&search_val='+refinesearch,
			success:function(data){
				$("#outer_div").html(data).fadeIn('slow');
				$("#loader").fadeOut('slow');
			}
		})

}

function deleteCategory(Ids){
	var site_url              = jQuery("meta[name='siteurl']").attr("content");
	var r = confirm("Are sure to delete category.");
	if (r == true) {
		x = "1";
	} else {
		x = "2";
	}
	if(x=="1") {
		$.post("ajax_all_add_edits.php", {command: 'DeleteCategoryList', Ids: Ids}, function (data, status) {
			if (jQuery.trim(data) == "1") {
				window.location = site_url + "all-category.php";
			}
		});
	}

}
