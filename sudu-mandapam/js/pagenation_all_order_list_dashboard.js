function load(page,serch_box_val,str_check,let_search,type_vals,ids_indst){
    ...
    $("#loader").fadeIn('slow');
    $.ajax({
        url:'get_all_order_details.php?action=ajax&page='+page+'&search_val='+serch_box_val+'&str_check_box='+str_check+'&searchd='+let_search+'&type_val='+type_val+'&ids_indst='+indst_val,
...
function fefine_search(){
     var refinesearch = $('#orderSearch').val();
    var check = "";
    $("#loader").fadeIn('slow');
    $.ajax({
        url:'get_all_order_details.php?action=ajax&search_val='+refinesearch+'&str_check='+check,
        success:function(data){
            $("#outer_div").html(data).fadeIn('slow');
            $("#loader").fadeOut('slow');
        }
...
function fefine_search1(){
    ...
    $("#loader").fadeIn('slow');
    $.ajax({
        url:'get_all_order_details.php?action=ajax&str_check='+refinesearch+'&search_val='+producattype1,
        success:function(data){
            $("#outer_div").html(data).fadeIn('slow');
            $("#loader").fadeOut('slow');
        }
