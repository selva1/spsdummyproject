...
<!-- header style start-->
    <body class="skin-black">
  	...
  	<div class="file-preview">
							...
							<?php    
							 $conn=dbconnection();
							 $stmt ="SELECT `id`, `product_Id`, `img_link`, `img_iden`, `status` FROM `tbl_productImg` WHERE product_Id=$edit_id and img_iden='thumbIMG'";
...
<div data-template="image" data-fileindex="-1" id="" class="file-preview-frame file-preview-success">
						 ...
						 <button title="Remove file" class="kv-file-remove btn btn-xs btn-default" type="button" onclick="deleteImgs('<?php echo $row['id']; ?>','<?php echo $row['img_link']; ?>');"><i class="glyphicon glyphicon-trash text-danger"></i></button>
						   ...
						   <?php } } else{ echo "No Big images found..."; } ?>
							...
							<div class="kv-fileinput-error file-error-message" style="display: none;"></div>
                              ...
                              <div class="col-lg-8">
                                   Default thumbnail Image ( dimension 250 X 250 )
                               
								    <input id="file2" name="file2[]" type="file" multiple=true class="file-loading">
                              </div>
  	...
  	<div class="file-preview">
							...
							<?php    
							 $conn=dbconnection();
							 $stmt ="SELECT `id`, `product_Id`, `img_link`, `img_iden`, `status` FROM `tbl_productImg` WHERE product_Id=$edit_id and img_iden='MainIMG'";
							$vid_comment =  mysqli_query($conn,$stmt);
							@$rowcount=mysqli_num_rows($vid_comment);
							if($rowcount>0){
...
<div data-template="image" data-fileindex="-1" id="" class="file-preview-frame file-preview-success">
						 ...
						 <button title="Remove file" class="kv-file-remove btn btn-xs btn-default" type="button" onclick="deleteImgs('<?php echo $row['id']; ?>','<?php echo $row['img_link']; ?>');"><i class="glyphicon glyphicon-trash text-danger"></i></button>
						   ...
						   <?php } } else{ echo "No Thumbnail images found..."; } ?>
     ...
     <script>
    $("#file").fileinput({
        uploadUrl: "get_products_image_upload.php?editId=<?php echo $edit_id; ?>&idef=thumbIMG", // server upload action
        uploadAsync: true,
        maxFileCount: 100
    });
    $("#file2").fileinput({
        uploadUrl: "get_products_image_upload.php?editId=<?php echo $edit_id; ?>&idef=MainIMG", // server upload action
        uploadAsync: true,
        maxFileCount: 100
    });
	...
	$.post("get_delete_product_img.php" , {command:'deleteProductImg',delId:delId,imgUrls:imgUrls} , function(data,status)	{
		if(jQuery.trim(data) == "true"){
			 window.location=site_url+"products_image_upload.php?editId=<?php echo $edit_id; ?>";
		}
	});	
