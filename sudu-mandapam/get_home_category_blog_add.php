...
$imageFileType = pathinfo($target_file,PATHINFO_EXTENSION);
 ...
 if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
	...
	$stmt = $conn->prepare("INSERT INTO home_categories_blog_list (`categories_id`, `cat_type_id`, `img_name`,`blog_name`,`blog_link`, `img_status`) VALUES (?, ?, ?, ?, ?, ?)");
$stmt->bind_param("iisssi", $categories, $mainselect, $dbimglink, $blog_name, $blog_link, $status);
$stmt->execute();
$conn->close();
}else{
...
$stmt = $conn->prepare("UPDATE home_categories_blog_list SET categories_id=?, cat_type_id=?,img_name=?,blog_name=?,blog_link=?,img_status=? WHERE id=?");
$stmt->bind_param('iisssii', $categories, $mainselect, $dbimglink,$blog_name,$blog_link,$status,$editsid);
$stmt->execute();
//echo "1";
$conn->close();
} else {
$stmt = $conn->prepare("UPDATE home_categories_blog_list SET categories_id=?, cat_type_id=?,blog_name=?,blog_link=?,img_status=? WHERE id=?");
$stmt->bind_param('iisssi',  $categories, $mainselect,$blog_name,$blog_link,$status,$editsid);
$stmt->execute();	
}

}
$redirecturl="location:".$siteAdminUrl."home-categories-blog-list.php";
header($redirecturl);
