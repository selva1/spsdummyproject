...
if($search_val!=""){
 $wherecaseselect = " and fpc.doctor_id='".$search_val."' "; 
	...
	$query = "SELECT `id`, `categories_id`, `cat_type_id`, `img_name`, `img_status` FROM `home_categories_top_banner_list` WHERE 1";
...
while($result = mysqli_fetch_array($result_final)){
		...
		<a href="add_home_categories_top_banner.php?editid=<?php echo $id; ?>"><button title="edit file" class="kv-file-upload btn btn-xs btn-default" type="button"><i class="glyphicon glyphicon-edit text-info"></i></button></a>
		<button title="Remove file" class="kv-file-remove btn btn-xs btn-default" type="button" onclick="deletecategorytopbanner('<?php echo $id; ?>');"><i class="glyphicon glyphicon-trash text-danger"></i></button>
		</div>
...
<?php
echo paginatecategoryTopBannerList($reload, $page, $total_pages, $adjacents,$search_val,$chek_box_val,$let_search,$type_val,$ids_indst);
} else {
