...
function load(page,serch_box_val,str_check,let_search,type_vals,ids_indst){
	...
	var ajaxSiteUrl = $("#ajaxSiteUrl").val();
		$.ajax({
			type: "POST",
			url:ajaxSiteUrl+'get_brands_list.php',
			data:'ajax=1&seo_1='+seo_1+'&seo_2='+seo_2+'&seo_3='+seo_3+'&ajax_task=get_brand_list&page='+page+'&search_val='+act+'&orderNumbers='+str_check+'&searchd='+let_search+'&allcolorVals='+type_val+'&allBandVals='+indst_val,
...
var refinesearch = document.getElementById("refinesearch").value;
	...
	$("#loader").fadeIn('slow');
		$.ajax({
			url:ajaxSiteUrl+'get_brands_list.php?action=ajax&search_val='+refinesearch,
			success:function(data){
				document.body.scrollTop = document.documentElement.scrollTop = 0;
				$("#loadCategorys").html(data).fadeIn('slow');
				$("#loader").fadeOut('slow');
...
$.ajax({
		type: "POST",
		url:ajaxSiteUrl+'get_brands_list.php',
		data:'ajax=1&seo_1='+seo_1+'&seo_2='+seo_2+'&seo_3='+seo_3+'&ajax_task=get_brand_list&allBandVals='+allBandVals+'&allcolorVals='+allcolorVals+'&sortBy='+sortBy+'&orderNumbers='+orderNumbers+'&fromPrice='+fromPrice+'&toPrice='+toPrice,
		success:function(data){
            document.body.scrollTop = document.documentElement.scrollTop = 0;
			$("#loadCategorys").html(data).fadeIn('slow');
			$("#loader").fadeOut('slow');
